...
const verifyToken = async (req, res, next) => {
  try {
    ...
    if (auth) {
      // verify device exists and token matches stored token to prevent reuse
      const device = await Device.findById(auth.id);
      if (!device) {
        return res.status(401).json({ success: false, message: 'Invalid session' });
      }
      if (!device.token || device.token !== token) {
        return res.status(401).json({ success: false, message: 'Token mismatch or session invalid' });
      }
      req.userId = auth.id;
      return next();
    }
