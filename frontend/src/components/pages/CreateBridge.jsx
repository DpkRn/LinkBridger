...
const CreateBridge = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      setLoading(true)
      const res=await api.post('/source/addnewsource',{userId:_id,source:platform,destination:profileLink},{withCredentials:true})
        ...
        {showBridge&&<div className="">
          <div className="px-4 py-2 bg-black/30 text-white rounded-md flex items-center break-words gap-3">
            <span className="break-all font-mono" ref={linkRef}>
              {`https://linkb-one.vercel.app/${_id}/${source}`}
            </span>
            <span>
              <MdContentCopy
